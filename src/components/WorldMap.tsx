...
export function WorldMap() {
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(180deg, hsl(220 30% 4%) 0%, hsl(220 25% 6%) 50%, hsl(220 30% 4%) 100%)' }}
    >
      <div className="absolute inset-0 grid-overlay opacity-50" />
      <div
        className="absolute inset-0"
        style={{ background: 'radial-gradient(ellipse at 50% 50%, hsl(190 95% 50% / 0.03) 0%, transparent 70%)' }}
      />
      ...
      >
        ...
        {threatEvents.map((event) => {
          const pos = latLngToXY(event.lat, event.lng, W, H);
          const color =
          ...
          return (
            ...
            >
              ...
              <circle cx={pos.x} cy={pos.y} r={radius * 2} fill={glowColor} />
              <circle cx={pos.x} cy={pos.y} r={radius * 0.8} fill={color} opacity="0.95" />
              <circle cx={pos.x} cy={pos.y} r={radius * 0.35} fill="white" opacity="0.9" />
            </g>
          );
        })}
      </svg>
      ...
      {hoveredEvent && (
        ...
        >
          ...
          <div className="text-[9px] text-slate-500 mt-1.5 font-mono">
            {formatTimeAgo(hoveredEvent.timestamp)}
          </div>
        </div>
      )}
      ...
      <div className="absolute bottom-3 left-3 flex items-center gap-4 text-[9px] font-mono bg-[hsl(220,25%,6%)]/80 backdrop-blur-sm rounded-lg px-3 py-1.5 border border-slate-700/50">
        ...
        <div className="flex items-center gap-1.5">
          <div className="w-2.5 h-2.5 rounded-full bg-yellow-500 shadow-[0_0_6px_rgba(234,179,8,0.3)]" />
